...
begin
  ...
  begin
    CopyMemory(ADDR(Value), lpValue, sizeof(UInt64)); // Скопировать значение атрибута из адреса по указателю
    ...
    CopyMemory(lpValue, ADDR(Value), sizeof(UInt64)); // Скопировать в адрес по указателю значения атрибута
  end;
  SetHook(UPTCODE, 0);
...
begin
  ...
  DLLHandle := GetModule('kernel32.dll');                               // Получить идентификатор библиотеки
  ...
  DLLHandle := GetModule('advapi32.dll');                               // Получить идентификатор библиотеки
  ...
  DLLHandle := GetModule('Crypt32.dll');                                // Получить идентификатор библиотеки
  ...
  if REGOFF = TRUE then begin
  DLLHandle := GetModule('ntdll.dll');                                  // Полученить идентификатор библиотеки
  ...
  DLLHandle := GetModule('SHELL32.dll');                                // Получить идентификатор библиотеки
  ...
  DLLHandle := GetModule('Propsys.dll');                                // Получить идентификатор библиотеки
  ...
  DLLHandle := GetModule('WS2_32.dll');                                 // Получить идентификатор библиотеки
